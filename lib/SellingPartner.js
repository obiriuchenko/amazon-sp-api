...
const xmlStreamParser = require('./xmlStreamParser');
...
class SellingPartner {
  ...
  // }
    ...
    this._xml_parser = new XMLParser({ numberParseOptions: { leadingZeros: false }});
  ...
  // Download a report or feed result, v2 version (streams) from https://github.com/starsellersworld/stream-amazon-sp-api
  // Options object:
  // json:false, // Optional: Whether or not the content should be transformed to json before returning it (from tab delimited flat-file or XML).
  // unzip:true, // Optional: Whether or not the content should be unzipped before returning it.
  // file:'<FILE_PATH>', // Optional: The absolute file path to save the report to. Even when saved to disk the report content is still returned.
  // charset:'utf8' // Optional: The charset to use for decoding the content. Is ignored when content is compressed and unzip is set to false.
  // returnType:'stream' // Optional: The type of the return value. Can be 'none', 'string' or 'stream'. Default is 'stream'.
  async downloadStream(details, options = {}) {
    options = Object.assign({
      unzip: true,
      returnType: 'stream', // 'none' ,'string', or 'stream'
    }, options);

    let content = null; // if filled then some transform is not-streamable

    let oFile = options.file ? fs.createWriteStream(options.file) : null;
    ...
    if (!details.compressionAlgorithm || options.unzip){
      ...
      if (options.json){
        ...
        if (hRes.headers['content-type'].includes('xml')) {
          // de-stream
          content = xmlStreamParser(iRes);
          // throw new CustomError({
          //   code: 'PARSE_ERROR',
          //   message: 'It is not possible XML to JSON and to stream together.',
          //   details: 'Please use either "json" or "stream" option for xml content or even "download" function instead.'
          // });
